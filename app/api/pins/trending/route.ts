...
export async function GET() {
  ...
  if (!readCounts?.length) {
    ...
    const { data: recent } = await supabase
      .from("pins")
      .select("id, headline, summary, stat_1, stat_2, stat_3, topic, topics, source_name, source_url, published_at, lat, lng, country_code, region_label, tickers, market_relevance")
      .not("lat", "is", null)
